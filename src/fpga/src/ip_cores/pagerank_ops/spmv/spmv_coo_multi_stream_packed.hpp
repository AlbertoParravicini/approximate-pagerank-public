...
void spmv_coo_multi_stream_packed(input_block *x, input_block *y, input_block *val,
...
#pragma HLS pipeline II=1
    ...
    #pragma HLS unroll
    		...
    		unsigned int y_curr_block = y_curr / BUFFER_SIZE;
    		unsigned int y_curr_offset = y_curr % BUFFER_SIZE;
    		unsigned int lower_range_y_curr = FIXED_WIDTH * y_curr_offset;
			unsigned int upper_range_y_curr = FIXED_WIDTH * (y_curr_offset + 1) - 1;
    		SCATTER_VEC_INNER: for(index_type k = 0; k < N_PPR_VERTICES; k++) {
    #pragma HLS unroll
    			unsigned int scattered_vec = vec[k][y_curr_block].range(upper_range_y_curr, lower_range_y_curr);
    			vec_local[k].range(upper_range, lower_range) = scattered_vec;
    		}
    	}
