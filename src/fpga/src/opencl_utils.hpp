...
struct ConfigOpenCL {
	...
	ConfigOpenCL(std::string kernel_name) : kernel_name(kernel_name) {
	}
	...
	cl::CommandQueue queue;  // compute command queue;
...
inline int setup_opencl_and_load_kernel(ConfigOpenCL &config, bool debug = true) {
	...
	config.queue = cl::CommandQueue(config.context, config.device, CL_QUEUE_PROFILING_ENABLE);
	...
	config.kernel = cl::Kernel(config.program, config.kernel_name.c_str());
