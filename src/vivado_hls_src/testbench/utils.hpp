...
template<typename I, typename T>
inline void readTuples(std::vector<I> *row_indices, std::vector<I> *col_indices, std::vector<T> *values, I nvals, FILE *f) {
	I row_ind, col_ind;
	T value = (T) 1.0;
...
inline int readMtx(const char *fname, std::vector<I> *row_indices, std::vector<I> *col_indices, std::vector<T> *values,
		int* num_vertices, int* num_edges, int directed = 1, bool debug = false) {
	int ret_code;
	MM_typecode matcode;
	FILE *f;
	...
	readTuples<I, T>(row_indices, col_indices, values, nvals, f);
	auto time_1 = chrono::duration_cast<chrono::milliseconds>(clock_type::now() - start_1).count();
	std::cout << "-- read tuples: " << time_1 / 1000 << " sec" << std::endl;
	...
	removeSelfloop<I, T>(row_indices, col_indices, values, &nvals);
	auto time_2 = chrono::duration_cast<chrono::milliseconds>(clock_type::now() - start_2).count();
	std::cout << "-- remove self-loops: " << time_2 / 1000 << " sec" << std::endl;

	auto start_3 = clock_type::now();
	customSort<I, T>(row_indices, col_indices, values);
	auto time_3 = chrono::duration_cast<chrono::milliseconds>(clock_type::now() - start_3).count();
	std::cout << "-- sort tuples: " << time_3 / 1000 << " sec" << std::endl;
...
inline void coo2csr(I *csrRowPtr, I *csrColInd, T *csrVal, const std::vector<I> &row_indices,
	...
	customSort<I, T>(&row_indices_t, &col_indices_t, &values_t);
...
template<typename I>
inline void readTuplesFast(std::vector<I> *row_indices, std::vector<I> *col_indices, I* nvals, FILE *f) {
	I row_ind, col_ind;
	...
	for (I i = 0; i < *nvals; i++) {
		...
		// Convert 1-based indexing MTX to 0-based indexing C++;
		if (row_ind != col_ind) {
			(*row_indices)[effective_nvals] = row_ind - 1;
			(*col_indices)[effective_nvals] = col_ind - 1;
			effective_nvals++;
		}
	}
...
inline int readMtxFast(const char *fname, std::vector<I> *row_indices, std::vector<I> *col_indices,
		I* num_vertices, I* num_edges, int directed = 1, bool debug = false) {
	int ret_code;
	MM_typecode matcode;
	FILE *f;
	...
	readTuplesFast<I>(row_indices, col_indices, &nvals, f);
