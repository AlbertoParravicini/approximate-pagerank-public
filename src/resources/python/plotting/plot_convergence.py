...
if __name__ == "__main__":
    ...
    graph_names = [f"|E|=~{get_exp_label(10**6)}", f"|E|=~{get_exp_label(2 * 10**6)}", "Amazon", "Twitter"]
   
    num_rows = 2
    num_col = len(res["V"].unique()) // num_rows
    fig = plt.figure(figsize=(2.0 * num_col, 2.2 * num_rows))
    ...
    sizes = [200000, 100000, 81306, 128000]
    
    MIN_ITER = 1
    MAX_ITER = 20
    max_iter_plot = [17, 17, 13, 19]
    ...
    plt.annotate("Convergence Error", xy=(0.05, 0.5), fontsize=12, ha="center", va="center", rotation=90, xycoords="figure fraction")

    fig.suptitle(f"PPR Convergence Error w.r.t\nfixed-point bitwidth",
                  fontsize=14, ha="left", x=0.05)
