...
//		}
		...
		dcg += (double) test_rel / log2(abs(golden_rel - DIM) + 2);
		idcg += (double) golden_rel / log2(abs(golden_rel - DIM) + 2);
...
unsigned int edit_distance(const std::vector<int> &s1, const std::vector<int> &s2)
{
